...
rb_registration = KeyboardButton(text='Регистрация')  # MODULE_14_5
...
rkb_markup.row(rb_buy, rb_registration)  # MODULE_14_5
...
# FSM 'Registration'
class RegistrationState(StatesGroup):  # MODULE_14_5
    username = State()
    email = State()
    age = State()
    balance = State()
...
# Handlers for FSM 'Registration'
@dp.message_handler(text='Регистрация')  # MODULE_14_5
async def sing_up(message: types.Message):
    await message.answer('Введите имя пользователя (только латинский алфавит):')
    await RegistrationState.username.set()


@dp.message_handler(state=RegistrationState.username)  # MODULE_14_5
...
@dp.message_handler(state=RegistrationState.email)  # MODULE_14_5
...
@dp.message_handler(state=RegistrationState.age)  # MODULE_14_5
